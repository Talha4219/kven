...
/** Bro's control surface. Shown instead of the plain terminal/files panel
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {tab === 'terminal' && (
          isFullscreenedHere ? (
            ...
            <Centered>Bro has no live terminal.</Centered>
          )
        )}
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  // The harness's own default model (Settings → default model). Bro and every
  ...
  const [dispatchTo, setDispatchTo] = useState<string>(''); // '' = Bro decides
  ...
  const restartWithModel = async (
    ...
    try {
      ...
      const hive = a.isGod
        ...
        : a.isAssistant
        ? { id: a.id, name: a.name, cwd: a.cwd, provider, isAssistant: true, role: "Bro's prep assistant" }
  ...
  const dispatch = async () => {
    ...
    setDispatchMsg(res.ok
      ? `sent to Bro${suggested ? ` (suggesting ${suggested.name})` : ''}`
      : `failed: ${res.error ?? '?'}`);
  ...
  const assignIssue = (issue: GHIssue) => {
    ...
    setDispatchTo(''); // Bro decomposes and assigns — no more broadcast blasts
  };
      ...
      <Section title="DISPATCH — VIA ARIA">
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          ...
          <Select value={dispatchTo} onChange={setDispatchTo}>
            <option value="">Bro decides</option>
        ...
        <textarea
          ...
          rows={2}
          placeholder="Describe the task… (Bro decomposes, writes the card, and assigns)"
          style={textareaStyle}
        />
            ...
            {a.isGod && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                ...
                <PixelButton
                  ...
                  onClick={async () => {
                    ...
                    if (engineProvider !== currentProvider) {
                      if (!window.confirm("This restarts Bro; a conversation on a different engine can't be resumed.")) return;
                    }
                ...
                <PixelButton
                  ...
                  onClick={() => restartWithModel(a, a.model, { resume: true })}
                >
                  <span title="Kill and respawn Bro, resuming the current conversation — fixes a corrupted/garbled terminal without losing context">
                    restart &amp; continue
                  </span>
                </PixelButton>
              </div>
