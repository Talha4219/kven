...
import { AriaBooting } from '@/components/AriaBooting';
...
export function App() {
  ...
  useEffect(() => {
    ...
    // Realtime Aria voice toggle can gate on it. Lives in the secret broker, not
  ...
  // hook) so Aria doesn't boot against the current home while the user may be
  ...
  // (Aria always spawns, so in normal operation it effectively never runs).
  useEffect(() => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minHeight: 0, minWidth: 0, position: 'relative' }}>
          <OfficeFloor />
          <MemoryPanel />
          {agentCount === 0 && godStatus === 'booting' && <AriaBooting />}
