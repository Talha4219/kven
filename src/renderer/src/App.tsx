...
import { BroBooting } from '@/components/BroBooting';
...
export function App() {
  ...
  useEffect(() => {
    ...
    // Realtime Bro voice toggle can gate on it. Lives in the secret broker, not
  ...
  // Shareable hires: a validated manifest arriving via the kven://
  ...
  // hook) so Bro doesn't boot against the current home while the user may be
  ...
  // (Bro always spawns, so in normal operation it effectively never runs).
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      {/* rt-12: global fixed-overlay toast for voice-Bro completions ("Baran
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minHeight: 0, minWidth: 0, position: 'relative' }}>
          <OfficeFloor />
          <MemoryPanel />
          {agentCount === 0 && godStatus === 'booting' && <BroBooting />}
