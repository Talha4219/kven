...
// The first thing Aria (god) is told on a fresh spawn — orient him and put
// him to work running the floor. Kept terse and action-oriented.
const INITIAL_GOD_PROMPT = [
  "You're online as Aria, the orchestrator of the hive. Get oriented, then start running the floor:",
...
function enrichTaskPrompt(text: string): string {
  return [
    ...
    'self-contained prompt to Aria via an outbox message with "to":"god". Do not do the task yourself.)'
  ].join('\n');
}
 ...
 *   1. spawns the god agent into Aria's room when none is running,
...
export function useHive(config: HarnessConfig | null): void {
  ...
  // never drains, so the "newest" id was frozen on it: Aria was nudged once per
  ...
  // Aria twice (the window between the listPtys check and spawnPty is racy).
  ...
  // Reactive so the assistant bootstrap (effect #1b) re-runs once Aria is ready.
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await window.cth.spawnPty({
        ...
        cols: 100,
        rows: 30,
        // Restore Aria's prior conversation across an app restart. His session
        ...
        resume: true,
        hive: { id: GOD_ID, name: 'Aria', provider: godProvider, cwd: config.harnessHome!, isGod: true, role: 'orchestrator (god)' }
      });
      ...
      // Kick Aria off once his TUI is up. Always re-enable remote control so
      ...
      // a RESUMED Aria already has his full context and must not be re-oriented
      ...
      // Aria until he's settled. The live-PTY branch above skips this entirely.
      const resumedGod = res.resumed === true;
      bootGraceUntil.current[GOD_ID] = Date.now() + BOOT_GRACE_MS;
      void (async () => {
        try {
          const remoteCommand = remoteControlCommandForProvider(godProvider, 'Aria');
          if (remoteCommand) {
  ...
  useEffect(() => {
    return window.cth.onHiveHookEvent((e) => {
      ...
      if (e.event === 'PreCompact') {
        ...
        // Only (1) is a real "needs you". Treating (2) as blocked made Aria
  ...
  // 5) Pipe inbound Slack messages into Aria's queue. The main-process Slack
  //    webhook server pushes each verified message here via IPC; enqueueing to
  //    GOD_ID lands it in Aria's queue exactly as if the user had typed it
    ...
    const revive = async (deadId: string): Promise<void> => {
      ...
      try {
        ...
        const hive = a.isGod
          ...
          : a.isAssistant
          ? { id: a.id, name: a.name, cwd, provider, isAssistant: true, role: "Aria's prep assistant" }
