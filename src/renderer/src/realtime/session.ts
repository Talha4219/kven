/**
 * Realtime Aria — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * loop at a time) exposed through a `useRealtimeAria()` hook via useSyncExternalStore.
 *
...
export interface RealtimeAriaState {
...
/** Warm openers Aria leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Aria here — what can I do for you?",
...
/** Aria's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Aria
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const ARIA_PERSONA =
  `You are Aria — the voice of the orchestrator ("god") of a hive of autonomous coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
/** The <audio> sink for Aria's voice. */
...
function setState(patch: Partial<RealtimeAriaState>): void {
  state = { ...state, ...patch };
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Aria';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Aria's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Aria's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Aria
    ...
    // Floor deltas — silent appends that keep Aria's picture live without
    ...
    // subscribe so a detected completion makes Aria speak it unprompted.
    ...
    // Open the conversation: have Aria speak a warm greeting as his first turn
...
function getSnapshot(): RealtimeAriaState {
  return state;
}

/**
 * React binding for the Realtime Aria voice loop. Returns the current state plus
 ...
 */
export function useRealtimeAria(): RealtimeAriaState & {
