/**
 * Realtime Bro — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 *
 * Phase 1 is a read-only connect→listen→respond round-trip. The agent runs Bro's
 * rt-4 READ-ONLY tools (get_fleet_status / get_tasks / get_cost / get_triggers /
 * get_config / get_memory / get_activity) and god's rt-6 "Bro" persona, so the
 ...
 * loop at a time) exposed through a `useRealtimeBro()` hook via useSyncExternalStore.
 *
...
export interface RealtimeBroState {
...
/** Warm openers Bro leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Bro here — what can I do for you?",
...
/** Bro's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Bro
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const ARIA_PERSONA =
  `You are Bro — the voice of the orchestrator ("god") of a hive of autonomous coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
- get_app_info — the Kven app itself: its version and the latest release notes. Use for "what version is this" or "what's new in this release".
...
HIVE VOCABULARY. Agents have an id like "rex-mqp3l5wn" and a friendly name like "Rex"; refer to them by name. "god" is the orchestrator whose voice you are. A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
/** The <audio> sink for Bro's voice. */
...
function setState(patch: Partial<RealtimeBroState>): void {
  state = { ...state, ...patch };
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Bro';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Bro's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Bro's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Bro
    ...
    // Floor deltas — silent appends that keep Bro's picture live without
    ...
    // subscribe so a detected completion makes Bro speak it unprompted.
    ...
    // Open the conversation: have Bro speak a warm greeting as his first turn
...
function getSnapshot(): RealtimeBroState {
  return state;
}

/**
 * React binding for the Realtime Bro voice loop. Returns the current state plus
 ...
 */
export function useRealtimeBro(): RealtimeBroState & {
