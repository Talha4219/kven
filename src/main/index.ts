...
// ─── Shareable hires: kven:// deep link + file import ───────────────────────
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('kven', process.execPath, [resolve(process.argv[1])]);
  }
} else {
  app.setAsDefaultProtocolClient('kven');
}
...
if (!gotInstanceLock) {
  ...
  app.on('second-instance', (_evt, argv) => {
    ...
    const link = argv.find((a) => a.startsWith('kven://'));
...
app.whenReady().then(() => {
  ...
  const startupHireLink = process.argv.find((a) => a.startsWith('kven://'));
