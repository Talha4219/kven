...
export interface AgentMeta {
  ...
  cwd: string;
  isGod?: boolean;
  /** Bro's prep assistant — enriches prompts and forwards them to Bro.
...
export class HiveManager {
  ...
  ensureHive(): void {
    ...
    // The Claude Code command reference Bro consults (refreshed each bootstrap
  ...
  async ensureAgent(
    ...
    opts: {
       ...
       *  is a no-op (tolerated until Bro populates the resource dir). */
      skillsDir?: string;
    } = {}
    ...
    // partial source dir is a no-op (Bro populates the resource dir in lp-manifest).
   ...
   * agent cwd (never whole-disk), and namespaces every id `kven-<id>` so a server
   ...
   * (Bro populates the resource dir in lp-manifest), and any IO error is swallowed
  ...
  private injectedPrompt(
    ...
    const godLine = meta.isGod
      ...
      : meta.isAssistant
      ? 'You are Bro\'s PREP ASSISTANT. You will be handed short, possibly vague instructions (each begins with "ENRICH TASK:"). For each one: (1) figure out which project it concerns and cd into the most relevant repo — you start in Bro\'s home directory; (2) gather concrete context READ-ONLY (exact file paths, current state, relevant code, conventions, active branch, gotchas) — NEVER modify, create, or delete files; (3) rewrite the instruction into ONE clear, self-contained prompt that Bro can execute autonomously, preserving the user\'s original intent without inventing scope. Then deliver it: write ONE message JSON into your outbox with "to":"god", "act":"request", a short subject, and the finished prompt as the body. Do NOT perform the task yourself — your only output is the improved prompt sent to Bro.'
   ...
   *  shim and merge a `kven-hive` hook group into agy's global hooks.json so a
  ...
  private installCodexHooks(dir: string): string {
    ...
    try {
      ...
      if (shim) {
        ...
        config += '\n# --- kven-hive lifecycle hooks (auto-generated; do not edit) ---\n';
  ...
  private installPiHooks(dir: string): string {
    ...
    try {
      ...
      const manifest = { name: 'kven-hive-bridge', version: '0.3.1', main: 'extensions/hive-bridge.js', auto: true };
  ...
  /** Write the live fleet snapshot Bro reads (`fleet.json`, gitignored).
   ...
   * store; its row is exactly the shape Bro (#4) reserves for the cost_ledger
  ...
  appendCostLedger(sample: AgentUsageSample): void {
    const root = this.root();
    if (!root) return;
    // Fully snake_case so the row maps 1:1 onto Bro's (#4) cost_ledger SQLite
